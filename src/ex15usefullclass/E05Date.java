...
public class E05Date {
	...
	public static String myDateFormat(Calendar c, String formatStr) {
		...
		/*
		getTime(): 서버의 표준시인 1970-01-01부터 현재까지의 시산을 초 단위로 반환해준다.
		이를 타임 스탬프라고 한다.
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		 ...
		 */
 		...
 		/*
 		 올해 크리스마스까지 남은 날짜를 계산한다. 여기서는 두가지 서식의 날짜를 파싱(추출)하여 구한 날짜객체를
 		 통해 계산한다.
 		 */
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			 ...
			 따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여 연산을 할수 있다.
			...
			/*
			 getTime()으로 얻어오는 시간은 13자리 정수값인데 10자리는 순수한 초(Sec)이고,
			 11~13자리는 밀리세컨즈를 의미한다. 즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한 초가 
			 반환된다. 또한 하루는 24*60*60로 864000초 이므로 아래와 같이 남은 날짜를 계산할 수 있다.
			 */
