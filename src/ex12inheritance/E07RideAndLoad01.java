...
오버로딩: 메서드명은 같지만 매개변수으 갯수, 타입, 순서가 다른 경우를
...
class C extends B {
	/*
	 * rideMethod()는 오버라이딩 된 메서드로 상위클래스에 정의된 멤버메서드와 반환타입, 메서드명, 매개변수의 갯수 및 타입이 모두
	 * 동일하다. 즉 똑같은 메서드이다.
	 */
	@Override
	public void rideMethod() {
	...
	/*
	 * loadMethod()는 오버로딩 된 메서드로 상위클래스에 정의된 멤버메서드와 메서드명만 동일할 뿐 매개변수의 타입, 갯수 등이 다르게
	 * 정의된다. 따라서 다른 메서드이다.
	 */
	...
	/*
	 * 따라서 C클래스로 인스턴스를 생성하면 오버라이딩 된 메서드는 모두 가려지게 되어 C클래스에 정의된 메서드 1개만 남게된다. 오버로딩 된
	 * 메서드는 서로 다르므로 3개 모두 남게된다.
	 */
}
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 기본 자료형의 경우에는 작은타입이 큰타입에 할당되는것은 자동형변환이 되기때문에 아래와 같이 작성할 수 있다. 클래스도 이와 마찬가지로
		 * 부모타입(큰타입)으로 자시인스턴스(작은타입) 을 참조할 수 있다.
		 */
		...
		/*
		 * 참조변수에 상관없이 항상 최하위에 오버라이딩된 메서드가 호출된다. 즉 아래 부분은 모두 C클래스에 정의된 rideMethod()가
		...
		/*
		 * C 타입의 참조변수를 통해서 인스턴스를 참조하면 모든 영역에 접근할 수 있다. 따라서 아래 3개의 문장은 모두 실행된다.
		 */
		ref3.loadMethod();
		ref3.loadMethod(1);
		ref3.loadMethod(1.1);
		...
		/*
		 * A 타입으로는 접근범위가 A클래스까지로 제한되므로 B와 C에 정의된 메서드는 호출할 수 없다.
		 */
		ref1.loadMethod();
		// ref1.loadMethod(1);
		// ref1.loadMethod(1.1);
...
//////////////////////////////////////////////////////////////////
		...
		//A -> C 참조하므로 가능
		A refId1 = new C();
		//B -> A를 참조하므로 불가능(자식은 부모를 참조할수 없다.)
//		B refId2 = refId1;
