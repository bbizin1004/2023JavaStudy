...
따라서 실제 업무에서도 유사한 기능으 클래스들을 묶어서 정의한다. 
 */
...
class Computer {
	...
	/* private 접근지정자로 선언된 멤버메서드는 외부크랠스 혹은 상속받은
	...
	/* private으로 선언된 멤버메서드를 외부에서 호출할 수 있게 public 으로 
	  지정하여 선언한 메서드로 "캡슐화" 되어있다.*/
...
/*
테블릿노트북: 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용펜으로
...
class TabletNotebook extends NotebookComputer {
	...
	//테블릿 사용을 위한 배터리량 체그 킻 등록된 펜인지 확인
	public void write(String penInfo) {
		...
		/*
		 ...
		 A.compareTo(B)
		 :문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열, -1 , 혹은 1이
		 반환되면 서로 다른 문자열로 판단하는 String클래스에 정의된 메서드
		 */
		...
		/*부모클래스에 정의된 메서드를 호출한다. 상속받은 자식 클래스에서는
		  이와같이 부모의 멤버메서드를 호출하여 자신의 멤버처럼 사용할 수 있다.
		  단 접근이 가능한 접근지정자로 선언되어야한다. super는
		  생략할 수 있다.*/
		super.movigCal();
