...
-인스턴스간에 형변환이 되려면 반드시 두 클리스간에 상속관계가 있어야한다.
-부모타입의 탐조변수로 자식 인스턴스를 참조할 수 있으므로
매개변수를 부모타입으로 만들면 자식 인스턴스를 인수로 받을 수 있다.
-매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로
정상적인 사용을 위해 강체형변환(다운캐스팅) 해야 하는데 이때 확인을 위해
사용된다.
 */

import ex12inheritance.PaperBox.GoldPaperBox;
...
public class E09InstanceOf {
	...
	/*
	-메서드 정의시 부모타입으 매개변수로 설정하면 자식타입으 인스턴스를
	모두 받을 수 있다.
	-Box타입으로 인수를 받게되면 모두 자동형변환(업캐스팅) 되므로 자식클래스의 메버메서느느
	호출할 수 없게된다.
	-이때 자식으 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이 필요하다.
	-instanceof 연산자를 통해 자식탕비으로 형변환이 가능한지 판단한 후 true를
	반환하면 다운캐스팅을 진행한다.
	 */
	static void wrapBox(Box b) {
		/*
		 세가지 인스턴스 중 어떤것이 전달되더라도 Box타입으로 자동형변환 되므로
		 해당 매개변수를 통해 호출할 수 있는 메서드는 b.boxWrap() 이 유일하다.
		 따라서 다운캐스팅이 필요하다.
		 */
	...
	/*
	만약 부모타입으 매개변수로 자식 인스턴스를 받을 수 없다면 아래와 같이
