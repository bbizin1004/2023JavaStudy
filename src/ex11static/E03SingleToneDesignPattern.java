...
2.그러면 클래스 외부에서는 생성자에 접근 할 수 없으므로 new를 통해
인스턴스 생성을 할수없다.
3.클래스 내부에 멤버변수를 static 으로 선언하여 인스턴스를 미리 생성해
놓는다.
...
class NoSingleTone {
	int instVar;
	/*생성자는 대부분 public으로 선언한다. 만약 private으로 선언하면  클래스외부 혹은
	 다른 패키지에서는 접근할 수 없기때문이다. */
	public NoSingleTone() {}
}
...
class SingleTone{
	//일반적인 멤버변수 선언
	int shareVar;
	/*정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JAM(자바
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		System.out.printf("st2의 주소:%s",st2);
		System.out.printf("st2의 주소:%s",st3);
		
		/*결국 동일한 인스턴스를 사용한 것이므로 마지막에 할당된
		 값이 출력된다.*/
		System.out.println(String.format("st2의 shareVar주소:%d",
				st2.shareVar));//200
		System.out.println(String.format("st2의 shareVar주소:%d",
				st3.shareVar));
