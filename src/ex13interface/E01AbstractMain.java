...
-인스턴스 생성은 할수없지만 그 외 참조변수, 상속 등의 모든 기능을 사용할 수 있다.

추상메서드
-오버라이딩의 목적으로 제작된다.
-메서드의 실행부가 없으므로 중괄호 대신 세미콜론으로 마무리한다.7
 */
...
/*
추상 메서드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항이 아닌 선택사항이다.
 즉 필요한 경우에만 오버라이딩 하면 된다. 아래 오버라이딩한 메서드는
 삭제해도 에러가 발생하지 않는다.
 */
...
public class E01AbstractMain {
	...
	public static void main(String[] args) {
		...
		/* 추상클래스내에 선언된 정적메서드는 오버라이딩의 대상이 될수 없으므로 
		 static의 기본규칙을 따른다. 별도의 생성절차 없이 클래스명으로 바로 접근할 수 있다.
		 */
