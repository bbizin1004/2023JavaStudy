...
/*
프로젝트 전체에서 기준으 역활을 하는 추상클래스 해당 클래스는 추상메서드를 
포함하고 있으므로 하위에서 상속받게되면 반드시 추상메서드를 오버라이딩 해야한다.
 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	public PersonalNumberStorageExt(int arrSize) {
		//매개변수로 전달된 정수를 통해 배열으 크기를 지정함
		personalArr = new PersonalInfoDTO[arrSize];
		numOfPerInfo = 0;
	}
	
	/*
	 상속을 통해 추상 메서드를 포함하게 되므로 반드시 아래와 같이 오버라이딩 해야한다.
	 이는 필수사항이며 하지않을 경우 에러가 발생하게된다.
	 */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for (int i = 0; i < numOfPerInfo; i++) {
			/*주민번호가 일치하는 판단한다. 주민번호는  private으로 선언된 멤버이므로
