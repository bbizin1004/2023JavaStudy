...
class Unit {
	...
	//내부클래스의 정의(내부 멤버 클래스)
	class Marine {
		//내부클래스도 일반적인 클래스와 동일하게 멤버를 정의할수 있다.
		int attackPower;
		/*내부 클래스에서는 외부클래스의 모든 멤버에 접근할 수 있다.
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/* 외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를
		 ...
		 생성이 되어 있으므로 아래와 같이 사용하 수있다. */
		Unit.Marine marine =new Unit("마린",50).marine;
		marine.attack();
		
		/*
		 Medic의 경우 Unit의 생성자에서 인스턴스 생성이 되지 않았으므로
		 아래와 같이 new를 통해 생성해야 한다. 이때에도  인스턴스를 통해
		 생성해야 하므로 new를 두번 사용해서 인스턴스를 생성한다.*/
